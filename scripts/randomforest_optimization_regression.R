...
write.csv(gtrain, paste0("reg_gtrain_",date,".csv"))
system(paste0("aws s3 cp reg_gtrain_",
              date,
              ".csv s3://earthlab-amahood/data/data_splits/reg_gtrain_",
              date,".csv"))
...
write.csv(gdev, paste0("reg_gdev_",date,".csv"))
system(paste0("aws s3 cp reg_gdev_",date,
              ".csv s3://earthlab-amahood/data/data_splits/reg_gdev_",date,".csv"))

gtest <- filter(gdevtest, split==FALSE) %>% dplyr::select(-split)
write.csv(gtest, paste0("reg_gtest_",date,".csv"))
system(paste0("aws s3 cp reg_gtest_",date,
              ".csv s3://earthlab-amahood/data/data_splits/reg_gtest_",date,".csv"))
...
write.csv(vtrain, paste0("reg_vtrain_",date,".csv"))
system(paste0("aws s3 cp reg_vtrain_",
              date,
              ".csv s3://earthlab-amahood/data/data_splits/reg_vtrain_",
              date,".csv"))
...
write.csv(vdev, paste0("reg_vdev_",date,".csv"))
system(paste0("aws s3 cp reg_vdev_",date,
              ".csv s3://earthlab-amahood/data/data_splits/reg_vdev_",date,".csv"))

vtest <- filter(vdevtest, split==FALSE) %>% dplyr::select(-split)
write.csv(vtest, paste0("reg_vtest_",date,".csv"))
system(paste0("aws s3 cp reg_vtest_",date,
              ".csv s3://earthlab-amahood/data/data_splits/reg_vtest_",date,".csv"))
...
hr <- foreach (i = 1:nrow(hyper_grid), .combine = rbind) %dopar% {
  ...
  m <- randomForest(formula = total_shrubs ~ ., 
                              data = train,
  ...
  #                       reference = dev1$total_shrubs)  # actual classes
    # Store OOB error for the model                      
  w<- data.frame(
    mtry = hyper_grid$mtry[i], 
    nodesize = hyper_grid$nodesize[i], 
    elevation = hyper_grid$elevation[i],
    rsq = m$rsq[length(m$rsq)],
    dataset = hyper_grid$dataset[i]
  )
  gc()
  system(paste("echo", round(m$rsq[length(m$rsq)],2), paste(round(i/nrow(hyper_grid)*100,2),"%")))
  return(w)
}

write.csv(hr, paste0("data/reg_hg",date,".csv"))
system(paste0("aws s3 cp data/reg_hg",date,".csv s3://earthlab-amahood/data/hypergrids_vb/reg_hg", date,".csv"))
