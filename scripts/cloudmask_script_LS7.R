...
for(year in years) {  # note that foreach has a slightly different syntax
  ...
  system(paste0("aws s3 sync s3://earthlab-amahood/data/landsat7/landsat_", year,
                " data/ls5/y",year,
                " --only-show-errors"))
  ...
  system(paste0("aws s3 cp",
                ...
                " s3://earthlab-amahood/data/needs7/needs_", year, ".csv",
                " --only-show-errors"))
  ...
  foreach(path_row_combo = prcs$prc)%dopar%{
    ...
    if(prcs[prcs$prc == path_row_combo,]$freq > 1){
      ...
      if(!file.exists(file.path("data/results/", filenamef))){
        ...
        tar_list <- Sys.glob(paste0(tar_path, "LE07", path_row_combo,"*.gz"))
        
        qas <- data.frame(filenames = NA, value66 = NA, goodpix = NA, i = NA)
        
        # deleting everything from previous loops
        system(paste0("rm -r data/scrap/",year,path_row_combo, "/*"))
        for (i in 1:length(tar_list)) {
          exdir <- paste0("data/scrap/",year,path_row_combo, "/",i)
          dir.create(exdir)
        ...
        for(i in 1:length(tar_list)){
          ...
          qa[[i]] <- raster(Sys.glob(paste0("data/scrap/",year,path_row_combo,"/", ordered_i[i][1],"/*pixel_qa.tif")))
        ...
        system(paste0("aws s3 cp",
                      ...
                      " s3://earthlab-amahood/data/landsat7_pixel_replaced/", filenamef))
